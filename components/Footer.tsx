...
const Footer: React.FC<FooterProps> = ({ onNavigate }) => {
  ...
  return (
    <footer className="bg-[#0b0c10] border-t border-white/5 pt-16 pb-8 relative overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-12 mb-12">
          ...
          <div className="col-span-1 md:col-span-2">
            <div className="flex flex-col gap-1 mb-6">
              <span className="font-en-display text-4xl font-black tracking-wide text-transparent bg-clip-text bg-gradient-to-r from-white to-gray-500">
                LINEHERO
              </span>
              <span className="text-xl font-bold text-hero-gold font-sans tracking-widest pl-1">無盡冒險</span>
            </div>
            <p className="text-gray-500 text-sm max-w-sm leading-relaxed border-l-2 border-white/10 pl-4">
              重新定義線上冒險 RPG。
              <br />
              在 LINE 聊天室中體驗最純粹的 MMORPG 文字冒險樂趣，與數萬名冒險者一同征戰。
            </p>
          </div>

          <div>
            <h4 className="text-white font-bold mb-6 uppercase tracking-[0.2em] text-xs">快速導覽</h4>
            <ul className="space-y-3 text-sm text-gray-400">
              <li><button onClick={() => onNavigate('game-intro-line')} className="hover:text-hero-gold hover:translate-x-1 transition-all text-left block">LINE 玩法介紹</button></li>
              <li><button onClick={() => onNavigate('game-intro-web')} className="hover:text-hero-gold hover:translate-x-1 transition-all text-left block">Web 玩法介紹</button></li>
              <li><button onClick={() => onNavigate('announcements')} className="hover:text-hero-gold hover:translate-x-1 transition-all text-left block">遊戲公告</button></li>
              <li><button onClick={() => onNavigate('shop')} className="hover:text-hero-gold hover:translate-x-1 transition-all text-left block">商城 / 儲值</button></li>
              <li><button onClick={() => onNavigate('office-games')} className="hover:text-hero-gold hover:translate-x-1 transition-all text-left block">辦公室玩家友善</button></li>
              <li>
                <a
                  href="https://tarven.linehero.tw"
                  target="_blank"
                  rel="noopener noreferrer"
                  className="hover:text-hero-gold hover:translate-x-1 transition-all text-left flex items-center gap-2"
                >
                  冒險者酒館
                  <svg className="w-3 h-3 text-hero-gold" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          ...
          <div>
            <h4 className="text-white font-bold mb-6 uppercase tracking-[0.2em] text-xs">條款與社群</h4>
            <ul className="space-y-3 text-sm text-gray-400 mb-6">
              <li><button onClick={() => onNavigate('privacy')} className="hover:text-hero-gold hover:translate-x-1 transition-all text-left block">隱私權政策</button></li>
              <li><button onClick={() => onNavigate('terms')} className="hover:text-hero-gold hover:translate-x-1 transition-all text-left block">服務條款</button></li>
              <li><button onClick={() => onNavigate('refund')} className="hover:text-hero-gold hover:translate-x-1 transition-all text-left block">退款政策</button></li>
            </ul>
            ...
            <div className="flex space-x-3">
              {socialLinks.map(link => (
                <a
                  ...
                  target="_blank"
                  rel="noopener noreferrer"
                  className="w-10 h-10 bg-white/5 rounded-lg flex items-center justify-center hover:bg-hero-gold hover:text-black transition-colors text-gray-400 border border-white/5 hover:border-hero-gold/50"
                >
                  {link.icon}
                </a>
              ))}
        ...
        <div className="border-t border-white/5 pt-8 flex flex-col md:flex-row justify-between items-center text-xs text-gray-600 gap-4">
          ...
          <div className="flex items-center gap-4 bg-black/30 px-4 py-2 rounded-full border border-white/5">
            <span className="flex items-center gap-1.5"><span className="w-1.5 h-1.5 rounded-full bg-green-500 animate-pulse"></span>Made in Taiwan</span>
